...
function ChatLabel({ openMenu, setOpenMenu }) {
  return (
    <div className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-lg text-sm group cursor-pointer'>
      ...
      <div className='group relative flex items-center justify-center h-6 w-6 aspect-square hover:bg-black/80 rounded-lg' onClick={() => setOpenMenu({ ...openMenu, open: !openMenu.open })}>
        <Image src={assets.three_dots} alt="" className={`w-4 ${openMenu.open ? "" : "hidden"} group-hover:block`} />
        <div className={`absolute w-4 ${openMenu.open ? "" : "hidden"} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}>
