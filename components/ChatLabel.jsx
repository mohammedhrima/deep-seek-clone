...
function ChatLabel({ openMenu, setOpenMenu, id, name }) {
  ...
  const selectChat = () => {
    ...
    // console.log("switch to chat with id", chatData._id); // Log the selected chat ID
  }
  ...
  return (
    <div onClick={selectChat}
      className={`mt-1 flex items-center justify-between p-2 text-white/80
               hover:bg-white/10 rounded-lg text-sm group cursor-pointer
               ${selectedChat._id == id && 'bg-white/10'}
               `}>
      ...
      <div onClick={e => { e.stopPropagation(); setOpenMenu({ id: id, open: !openMenu.open }) }}
        ...
        <div className={`absolute w-4 ${openMenu.id === id && openMenu.open ? "" : "hidden"} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}>
          <div onClick={renameHandler} className='flex items-center gap-3 hover:bg-white/10 px-3 py-2 rounded-lg'>
            <Image src={assets.pencil_icon} alt="" className="w-4" /><p>Rename</p>
          </div>
          <div onClick={deleteHandler} className='flex items-center gap-3 hover:bg-white/10 px-3 py-2 rounded-lg'>
            <Image src={assets.delete_icon} alt="" className="w-4" /><p>Delete</p>
          </div>
        </div>
      </div>
    </div>
